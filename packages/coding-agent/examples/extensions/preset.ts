 ...
 * - ~/.config/rpi/agent/presets.json (global)
 * - <cwd>/.config/rpi/presets.json (project-local)
 *
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".config", "rpi", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.config/rpi/agent/presets.json or .config/rpi/presets.json",
				"warning",
			);
			return;
		}
