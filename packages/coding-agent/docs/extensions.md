...
> **Placement for /reload:** Put extensions in `~/.config/rpi/agent/extensions/` (global) or `.config/rpi/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.config/rpi/agent/extensions/my-extension.ts`:
...
| Location | Scope |
|----------|-------|
| `~/.config/rpi/agent/extensions/*.ts` | Global (all projects) |
| `~/.config/rpi/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.config/rpi/extensions/*.ts` | Project-local |
| `.config/rpi/extensions/*/index.ts` | Project-local (subdirectory) |
